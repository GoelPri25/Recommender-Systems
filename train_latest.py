...
from dataset_load import simple_load_data_rate, get_model_data
...
base_dir = os.getcwd()
# name_rating_dir = "../ml-1m/rateings.dat"
...
rating_data_file = os.path.join(os.path.dirname(base_dir), "ml-1m", "ratings.dat")


#train_dict, val_dict, test_dict, non_interacted_movies, movie_num, user_num = load_data_rate(file_name)

NEGATIVE_SAMPLING = [3, 5, 10]
for negative_sample_no in NEGATIVE_SAMPLING:
        
    train_dict, valid_dict, test_dict, movie_num, user_num, removed_users_info= simple_load_data_rate(rating_data_file, negative_sample_no_train=negative_sample_no, negative_sample_no_valid=100, threshold=4)
    ...
    for layers in LAYERS:
        FACTOR_DIM = layers[-1]
        print(layers, FACTOR_DIM)
        layer_name = "_".join(map(str, layers))
        print(layer_name)  
        ...
        model_ncf = NeuMF(
            ...
            mf_dim=FACTOR_DIM,       # Larger embeddings (vs. 10) for richer latent features
            layers=layers, # Deeper MLP for nonlinear interactions
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                ...
                df_metrics.to_csv(f'./training_metrics_without_scheduler_withLimit_{layer_name}_layer_{negative_sample_no}_.csv', index=False)
        
                # Early stopping mechanism
                tolerance = 0.001  # Allow minor fluctuations
                if val_loss_avg < (best_val_loss - tolerance):
                    best_val_loss = val_loss_avg
                    torch.save(model_ncf.state_dict(), f"./best_model_ncf_{layer_name}_layer_{negative_sample_no}_.pth")
        ...
        torch.save(model_ncf.state_dict(), f"./_model_ncf_{layer_name}_layer_{negative_sample_no}_.pth")
        # Test phase
        ...
        with torch.no_grad():
            ...
            df_metrics = pd.DataFrame(metrics)
            df_metrics.to_csv(f'./training_metrics_with_test_results_{layer_name}_layer_{negative_sample_no}_.csv', index=False)
