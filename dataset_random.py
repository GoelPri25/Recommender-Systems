...
def simple_load_data_rate(filename, negative_sample_no_train=10, negative_sample_no_valid=100, threshold=3, train_ratio=0.7, test_ratio=0.15):
    ...
    for user_id, interactions in user_ratings.items():
        ...
        #try:
        val_dict[user_id] = val_pos[:5] + val_neg[:50]
        # print(len(val_neg))
        if len(val_pos) < 5 or len(val_neg)<50:
            print("yes test")
            print(len(val_pos), len(val_neg))
        
        test_dict[user_id] = test_pos[:5] + test_neg[:50]
        if len(test_pos) < 5 or len(test_neg)<50:
            print("yes test")
            print(len(test_pos), len(test_neg))

        if len(train_pos) > len(train_neg):
            print("Yup train_pos bigger than train_neg")
            print(len(train_pos), len(train_neg))
            deficit = len(train_pos) - len(train_neg)  # How many negatives are needed
    
            # Take negatives from val_neg and test_neg if available
            extra_negatives = val_neg[50:] + test_neg[50:]  # Remaining negatives
            additional_neg = extra_negatives[:deficit]  # Take only as many as needed
        
            # Add the additional negatives to train_neg
            train_neg.extend(additional_neg)
            if len(train_pos) > len(train_neg):
                print("not fixed")
            train_dict[user_id] = train_pos + train_neg
