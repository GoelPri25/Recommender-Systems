...
def calculate_ndcg(relevant_items, top_k_items, K=10):
    """
    Calculate NDCG (Normalized Discounted Cumulative Gain) for a user.
    """
    dcg = 0.0
    idcg = 0.0
    for i, item in enumerate(top_k_items):
        if item in relevant_items:
            dcg += 1 / np.log2(i + 2)  # Discount factor (i+2 for 1-based index)
        if item in relevant_items:
            idcg += 1 / np.log2(i + 2)  # Ideal DCG: all relevant items in top K
...
def model_evaluation_metric(model, val_dict, device, K=10, batch_size=1024):
    ...
    recall_list, ndcg_list, precision_list = [], [], []  # Adding precision_list
    for u, interactions in val_dict.items():
    ...
    avg_precision_at_10 = np.mean(precision_list) if precision_list else 0  # Calculate avg precision@10
